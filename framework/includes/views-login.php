...
class APP_Login extends APP_Login_Base {
	...
	function process_form() {
		...
		$result = $wpdb->get_results('select Status from '.$wpdb->prefix.'users where user_login="'.$_REQUEST['log'].'"',ARRAY_A);		
	...
	//	echo $redirect_to;
		...
		{
			$redirect_to = site_url().'/login?action=notactivated';
			wp_safe_redirect($redirect_to);
		}
...
class APP_Registration extends APP_Login_Base {
	...
	function register_new_user() {
		...
		$posted = array();
		$errors = new WP_Error();
		$user_pass = $_POST['pass1'];//wp_generate_password();
		...
		$activation_link = get_bloginfo('siteurl').'/login?action=register&result=success&value=123456789dfdfjfjeifjfnf&id='.$user_id.'&val=RRRR_adfsdfdsfdsf_gg';
		
		
		$message = '<html><body><div >Hello '.$posted['user_login'].','.

																'<br><br>Welcome to '.get_bloginfo('name').', your leader in online classified ad listings.                    									<br><br>Your username: '.$posted['user_login'].

																'<br>Your password: '.$_POST['pass1'].
																
																'<br><br>Please <a href="'.$activation_link.'">Click here</a> to activate your account '.PHP_EOL.

																'<br><br>If you have any questions, please just let us know.<br><br>Best regards,<br>Your Greenaira - Nigeria  Free Classified Advert Site Team<br>'.site_url().

																'</body></html>';
			
			$headers = array('Content-Type: text/html; charset=UTF-8');
		//appthemes_send_email( $to, $subject, $message ); 
		...
		if ( $show_password_fields ) {
			...
			if ( isset( $_REQUEST['redirect_to'] ) ) {
				...
				$success_redirect = site_url().'/register?action=email_acti'; }
			else
				$success_redirect = site_url().'/register?action=email_acti';
		} else {
