...
$useragent=$_SERVER['HTTP_USER_AGENT'];
		...
		if ( $image_id > 0 ) {
			...
			$adthumbarray = wp_get_attachment_image( $image_id, 'large' );
...
function cp_do_update_geocode( $meta_id, $post_id, $meta_key, $meta_value ) {
	...
	if ( in_array( $meta_key, array( 'cp_city', 'cp_country', 'cp_state', 'cp_street', 'cp_zipcode', 'cp_business_phone', 'cp_business_name', 'cp_business_email', 'cp_business_address', 'cp_bb_pin', 'cp_website_url' ) ) ) {

		$result = $wpdb->get_results( $wpdb->prepare( "SELECT meta_key, meta_value FROM $wpdb->postmeta WHERE post_id = %d AND meta_key IN ('cp_street','cp_city','cp_state','cp_zipcode','cp_country','cp_business_phone','cp_business_name','cp_business_email','cp_business_address','cp_bb_pin','cp_website_url')", $post_id ), OBJECT_K );
